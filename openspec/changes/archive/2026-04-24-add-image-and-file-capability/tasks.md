...
- [x] 7.1 Modify `handleSimpleXEvent` in `kawa.ts` to handle `rcvFileStart`, `rcvFileComplete`, `rcvFileSndCancelled` events — delegate to `FileReceiver`. **Note**: The SimpleX SDK does not expose `rcvFileCancelled` as a separate `ChatEvent` type; receiver-initiated cancellations are detected via `newChatItems` with `CIFile.fileStatus === "rcvCancelled"` (see verify findings). `sndFileComplete` is handled implicitly by the SDK and does not require an event handler in Kawa.
...
- [x] 7.3 Modify `extractTextFromContent` in `event-formatter.ts` to also extract text from `MsgContent.Image` (caption text), `MsgContent.Video` (caption text), `MsgContent.File` (file name + text), and `MsgContent.Link` (link text) — return the text portion plus a marker that a file is attached. Note: the SimpleX SDK's received `MsgContent.Image` and `MsgContent.Video` types do not declare a `.text` property in TypeScript, but the runtime protocol may include it. The implementation uses runtime property access with fallbacks (`msgContent.text || "📷 Image"`) rather than direct type-casts to avoid type-safety violations.
